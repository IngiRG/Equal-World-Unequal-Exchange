...
let rows=[], layer="gap", scenario="ewa", region="world", worldData=null;
...
function draw(world){
 ...
 const shown=rows.filter(d=>region==="world"||region==="compare"||String(d.region_group||"").toLowerCase()===region); const vals=shown.map(d=>metric(d)); const extent=d3.extent(vals.map(Math.abs)); const max=Math.max(...vals.map(Math.abs),1);
 ...
 svg.selectAll("path").data(features).join("path").attr("class",d=>"country "+(by.has(isoNum[d.id])?"hasdata":""))
  .attr("d",path).attr("fill",d=>{const r=by.get(isoNum[d.id]);return r?color(metric(r)):null})
  .on("click",(e,d)=>{const iso=isoNum[d.id];if(iso) loadCountry(iso)})
  .on("mousemove",(e,d)=>{const r=by.get(isoNum[d.id]);if(!r)return; const t=document.getElementById("tooltip");t.style.display="block";t.style.left=(e.offsetX+12)+"px";t.style.top=(e.offsetY+12)+"px";t.textContent=`${r.country} · ${r.region_group||""}: ${fmt(metric(r))}`})
  .on("mouseleave",()=>document.getElementById("tooltip").style.display="none");
 document.getElementById("legend").textContent=(scenario==="hickel"?"Hickel-style":"EWA")+" · "+(region==="world"?"World":region)+" · empirical data when available";
}
...
function metric(r){if(scenario==="hickel"&&layer==="gap")return r.hickel_gap_eur_per_hour??r.gap;if(layer==="actual")return r.actual_hourly_comp_eur??r.actual;if(layer==="equal")return scenario==="hickel"?(r.hickel_hourly_comp_eur??r.equal):(r.equal_hourly_comp_eur??r.equal);return r.wage_gap_eur_per_hour??r.gap}
